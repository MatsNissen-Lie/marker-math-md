...
$$S.T. \quad \sum_{S \ni i} \sum_{j \in N} y(S, j) \le 1 \qquad \forall i \in M$$
...
$$P\begin{pmatrix} h_x^1(\tilde{\xi})x + h_y^1(\tilde{\xi})y \le 2 \ 400 \\ h_x^2(\tilde{\xi})x + h_y^2(\tilde{\xi})y \le 2 \ 400 \end{pmatrix} \ge \alpha$$
 (C1)
...
$$y_{ik} = \begin{cases}
1, \text{ if customer } i \text{ is visited by vehicle } k \\
0, \text{ otherwise}
\end{cases}$$
...
$$\sum_{i=1}^{N} D_i y_{ik} \leq B, \qquad k = 1, ..., K, \qquad \text{Each vehicle visit clients whose demand is under the capacity}$$

$$\sum_{i=0}^{N} x_{ijk} = y_{jk}, \qquad j = 0, 1, ..., N; k = 1, ..., K, \qquad \begin{cases} \text{Flow} \\ \text{constraints,} \\ \text{couples x and y} \end{cases}$$
...
$$\bar{F}_{ik} = \text{antall kunder i rute } k \text{ før ødelegging}.$$
...
$$\sum_{i \in \mathcal{N}} y_{ik} = \bar{E}_k, \quad k = 1,\dots,K$$
(2)
