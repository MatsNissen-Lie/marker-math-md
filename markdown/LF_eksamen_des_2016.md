...
- Vi kan definere = − . Da er en iid random variabel med �� = −
- Vi kan utføre en hypotesetest for å sjekk om ≠
- Hypotesetest 0: �� = 0, 1: �� ≠ 0
...
Beslutningsvariabler
 ...
 $x_r$  – 1 hvis rute r velges, 0 ellers

 $z_{v}^{s}$  – Antall passasjerer som må fraktes med taxi på veistrekning v i scenario s
...
$$\sum_{r \in R_v} Q_r \cdot x_r + z_v^s \geq P_v^s \qquad v \in V, s \in S$$

$$x_r \in \{0,1\} \quad r \in R$$

$$z_v^s \geq 0 \qquad v \in V, s \in S$$

b) Tilleggsnotasjon:

Mengder

*Rv* – Et sett med alle ruter *r* som inneholder veistrekning *v, Rv* ⊆ *R*  Beslutningsvariabler

*xr* – 1 hvis rute *r* velges, 0 ellers

Chance constraint formulering:

$$\min \sum_{r \in R} C_r \cdot x_r$$
...
a) En løsning, , kan sies å være Pareto-optimal dersom det ikke eksisterer en annen løsning, , som dominerer . For at skal dominere må 1) løsning være minst like god som målt med alle kriterier (objektiver), og at 2) er strengt bedre enn for minst ett av kriteriene,

$$\operatorname{dvs.} y \prec ff \left\{ \begin{aligned} &f_i(y) \leq f_i(x), & \forall i \in 1, 2, \dots, k \\ \exists j \in 1, 2, \dots, k & f_j(y) < f_j(x) \end{aligned} \right\}.$$

Det må altså ikke finnes noen slike løsninger som tilfredstiller det over for at vi kan se at en løsning er Pareto-optimal.

b) I *ε-constrained*-metoden velger vi å la én av objektivene bli igjen i målfunksjonen, mens de andre går inn som restriksjoner, én for hvert av de andre objektivene, hvor vi setter en øvre/nedre grense for hver av disse (angitt med *ε*). Ved å løse problemet som en singelobjektiv-problem en rekke ganger for ulike verdier av *ε*, kan en generere en Pareto-front.

I dette problemet kan vi f.eks. velge å la 1 være igjen i målfunksjonen, mens vi legger til følgende restriksjon for 2 (sammen med de andre restriksjonene):

$$z_2 = R_1 x_1 + R_2 x_2 + R_3 x_3 \ge \varepsilon$$

Her blir det en ≥-restriksjon fordi vi ønsker å maksimere 2 i motsetning til i forelesningsnotatene hvor vi så på min-problem. Ved å løse dette for en rekke verdier av *ε*, vil en få generert en Pareto-front.

c) Vektet sum-metoden vil ikke være i stand til å finne løsninger i den konvekse delen av Pareto-fronten (den konvekse delen fordi vi ønsker å maksimere både 1 og 2, mens dersom vi ønsket å minimere, ville det vært den konkave delen, som i forelesningsnotatene).

![](_page_3_Figure_11.jpeg)

Figuren over illustrerer hvilke løsninger som ligger i den konvekse delen av Pareto-fronten, angitt med X, en ikke vil være i stand til å finne med vektet sum-metoden. Årsaken til dette er at vektingen som benyttes i denne metoden gjør at en i stedet vil finne løsningene i endene av de stiplede linjene i figuren over i stedet for de som ligger under (angitt med X). *ε-constrained*-metoden har ikke denne begrensningen, så en kan derfor fastslå at det er den metoden som er benyttet i denne oppgaven.

d) Evolusjonære (Genetiske) algoritmer har den fordelen, i motsetning til f.eks. både *εconstrained*-metoden og vektet sum-metoden, at den er populasjonsbasert. Det betyr at den til enhver tid opererer med et sett av løsninger som endrer seg fra en iterasjon til en annen. Det betyr igjen at en kan generere en approksimasjon av Pareto-fronten med én kjøring av algoritmen, mens for de tidligere nevnte metodene må en ha flere kjøringer (f.eks. én for hver verdi av *ε* eller hver kombinasjon av vektene). Spesielt dersom det er mange objektiver vil det kreve mange kjøringer ved bruk av både *ε-constrained*-metoden og vektet summetoden for å får opp hele Pareto-fronten.

I motsetning til f.eks. vektet sum-metoden (men som for *ε-constrained*-metoden) er Evolusjonære (Genetiske) algoritmer mindre sensitiv til formen på Pareto-fronten. De krever gjerne også mindre kunnskap om selve problemet som løses, mens en i vektet sum-metoden og *ε-constrained*-metoden må vite en del om problemet for å kunne sette fornuftige verdier på hhv. vektene og verdiene på *ε*.
